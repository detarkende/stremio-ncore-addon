...
export abstract class TorrentDetails implements ParsedTorrentDetails {
  ...
  public getMediaFileIndex({ season, episode }: Pick<StreamQuery, 'season' | 'episode'>): number {
    ...
    const parsedFileNames = this.files.map((file) => filenameParse(file.name, true) as ParsedShow);
  ...
  public getFile({ season, episode }: Pick<StreamQuery, 'season' | 'episode'>): TorrentFileDetails {
